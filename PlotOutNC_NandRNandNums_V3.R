...
#
 ...
 #DateRun ="Aug_1"
 #folders = c("Aug_1_FIXPREY_V2_NoF_Starve")
...
DateRun ="2014"
folders = c("Sep10_PRM54")#, "OUT_AtlantisGOC_30yr_NoF - 2","OUT_AtlantisGOC_30yr_NoF - 3","OUT_AtlantisGOC_30yr_NoF - 4","OUT_AtlantisGOC_30yr_NoF - 5","OUT_AtlantisGOC_30yr_NoF - 6","OUT_AtlantisGOC_30yr_NoF - 7","OUT_AtlantisGOC_30yr_NoF - 8")#,"Jul_26_FIXPREY_V2_NoF_Starve_3")

 #pathfiles="F:/Atlantis/Results_NEW/"
 #DateRun ="245_ER_365" 
 #folders = c("245_ER_365")#,"OUT_Atlantis2_V2","OUT_Atlantis2_V3","OUT_Atlantis2_V4","OUT_Atlantis2_V5","OUT_Atlantis2_V6","OUT_Atlantis2_V7","OUT_Atlantis2_V8")
...
fishingStartYear<-1985 #All this does is draw a red vertical line on the plots. 
numStepsPerYear<-0.5  #Number of output intervals per year. With output intervals of 122 days, this is = 3. 
...
{ 
  ...
  #MAKE THE PLOT
        ...
        plot(simYears,thisY,lwd=2,type='l',cex.lab=1,cex.axis=1, col='black',lty='solid',ylab=yLabString,main=this.name$Name,xlab='Year',ylim=c(0,maxYForPlot ),cex.main=1)
        if (wantSubtitle)
        {
          title(sub=paste(this.name$CODE,"Biom ratio=",Bratio,sep=" "))
        }
        lines(x=c(0,max(simYears)),y=c(unfishedB,unfishedB),lty='dashed',lwd=1.5,col='red')
      lines(x=c(0,max(simYears)),y=c(X1985B,X1985B),lty='solid',lwd=1.5,col='blue')
